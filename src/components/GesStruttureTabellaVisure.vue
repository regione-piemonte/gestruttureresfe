  ...
  >
    ...
    <template v-slot:top>
      <q-input
        ...
        class="full-width"
        autofocus
        hint="Puoi cercare per codice fiscale, cognome e nome ospite o personale, cognome e nome medico MMG o cittÃ  di domicilio"
      >
    ...
    <template v-slot:body-cell-icons="props">
      <q-td :props="props">
        <mmg-subject-list-table-row-icons :row="props.row" />
      </q-td>
    </template>
...
<script>
import MmgSubjectListTableRowIcons from 'components/mmgComponents/MmgSubjectListTableRowIcons'

export default {
  name: 'GesStruttureTabellaVisure',
  components: {
    MmgSubjectListTableRowIcons
  },
  props: ['columns', 'data', 'idStrutturaRes'],
  data () {
    return {
      backDataFilter: null,
      searchValue: '',
  ...
  methods: {
    onRowClick (evt, row) {
      this.$router.push('/visure/dettaglio/' + row.idSoggetto + '/' + this.idStrutturaRes)
    },
    search () {
