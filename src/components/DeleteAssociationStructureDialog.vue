      ...
      <q-form ref="form" greedy @submit.prevent>
        <q-card-section class="q-py-none">
          <q-toolbar class="q-px-none">
            <q-toolbar-title>Movimento uscita da struttura</q-toolbar-title>
        ...
        <template v-if="!isLoading">
          <q-card-section>
            Desideri rimuovere l'assegnazione del soggetto
        ...
        <template v-if="!isLoading">
          <q-card-section v-if="subjectStrutturaAssistenza == strutturaOperazione">
            <div class="row col-md-12">
              <div class="col-md-5 q-pr-lg">
                <q-input class="col-md-4" v-model="dataFine" type="date" label="Data Uscita*" stack-label
              ...
              <div class="col-md-2 q-pr-lg">
                <q-toggle v-model="flag" label="Preserva Posto" />
              </div>
              <div class="col-md-12 q-pr-lg" v-if="dest && dest.length > 0">
                <q-select class="col-md-4" v-model="destinazioneSelezionata" :options="struttureOptions"
                  option-value="idStruttura" option-label="nome" label="Destinazione" emit-value map-options dense filled
...
import { disassociaSoggettoStruttura, getMotivazioneFullList, getStruttureByNatura } from "../services/api";
import { notifyError, notifySuccess, sortBy } from "../services/utils";
import { date } from 'quasar';

export default {
  name: "DeleteAssociationStructureDialog",
  props: {
