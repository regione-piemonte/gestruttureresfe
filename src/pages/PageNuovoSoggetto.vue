...
<template>
  <q-page padding>
    <ges-strutture-page-title ref="title" @back="onBack"
      >Nuovo soggetto {{ nomeStruttura }}</ges-strutture-page-title
    >
          ...
          <q-tab-panel :name="SEARCH_TABS.TAX_CODE">
            <q-form ref="formSearchByTaxCode" greedy @submit="noop">
              <div class="row items-center q-col-gutter-md">
                ...
                <div class="col-12">
                  <q-input
                    :value="auraSearchTaxCode"
                    :rules="[ruleRequired]"
                    @input="updateAuraSearchTaxCode"
                    type="text"
    ...
    <!-- ----------------------------------------------------------------------------------------------------------- -->
    <q-form
      v-if="isSearchedOnAura && !isSearchingOnAura"
      ref="formSoggetto"
      greedy
      @submit="noop"
    >
      <q-card class="q-mt-md">
          ...
          <div class="row items-center q-col-gutter-md">
            ...
            <div class="col-12 col-md-6 col-lg-4">
              <q-input
                v-model="lastName"
                type="text"
                @input="dataSoggNotValid = false"
                label="Cognome*"
                filled
                :disable="isSubjectOnSystem || isSubjectOnAura"
                dense
                no-error-icon
                :rules="[ruleRequired]"
              />
            ...
            <div class="col-12 col-md-6 col-lg-4">
              <q-input
                ...
                dense
                v-model="taxCode"
                :disable="(isSubjectOnSystem || isSubjectOnAura) && !!codFisc"
                no-error-icon
                maxlength="16"
                counter
                bottom-slots
            ...
            <div class="col-12 col-md-6 col-lg-4">
              <q-select
                ...
                :disable="(isSubjectOnSystem || isSubjectOnAura) && !!codFisc"
                :rules="[tipoDocumentoRule]"
                option-value="documentoTipoId"
                option-label="documentoTipoDesc"
              ...
              <q-banner rounded class=" bg-blue-2 ">
                <div>
                  <q-checkbox
                    class="text-body1"
                    v-model="certified"
                    label="Si dichiara di aver provveduto ad informare  il cittadino (ai sensi della normativa privacy)
            ...
            <div class="col-12">
              <q-select
                filled
                v-model="subjectTypeSelected"
                :options="subjectTypeOptions"
                label="Tipo soggetto*"
                option-label="descTipoSoggetto"
                option-value="idTipoSoggetto"
                emit-value
                map-options
                dense
                @input="
            ...
            <div class="col-12 col-md-6">
              <q-select
                filled
                v-model="aslResidenceSelected"
                :options="aslOptions"
                :disable="aslSelectDisabled"
                label="ASL di residenza*"
                option-label="descAslEstesa"
                option-value="descAslEstesa"
                emit-value
                @input="dataSoggNotValid = false"
                map-options
                dense
                ...
                bottom-slots
                clearable
                hint="Per i soggetti fuori regione puoi selezionare ''Extra regione''"
                :loading="isLoading"
                :rules="[ruleRequired]"
              />
            </div>
            ...
            <template v-if="isAsrDomicileVisible">
              <div class="col-12">
                <q-select
                  ...
                  "
                  v-model="asrDomicileSelected"
                  :options="asrOptions"
                  label="ASR di assegnazione*"
            ...
            <div class="col-12 col-md-6 col-lg-4">
              <q-select
                filled
                :disable="isSubjectOnSystem || isSubjectOnAura"
                v-model="cityResidenceSelected"
                :options="cityResidenceOptions"
                :label="`Comune residenza${comuneObbligatorio ? '*' : ''}`"
                ...
                clearable
                :loading="isLoading"
                @filter="onFilterCityResidenceOptions"
                :rules="[residenceRule]"
              />
            </div>
            ...
            <div class="col-12 col-md-6 col-lg-4">
              <q-select
                filled
                v-model="cityDomicileSelected"
                :disable="isSubjectOnSystem || isSubjectOnAura"
                :options="cityDomicileOptions"
                :label="`Comune domicilio${comuneObbligatorio ? '*' : ''}`"
                option-label="nomeComune"
                option-value="istatComune"
                dense
                options-dense
                ...
                clearable
                :loading="isLoading"
                @filter="onFilterCityDomicileOptions"
                :rules="[domicileRule]"
              />
            </div>
        ...
        <label v-if="dataSoggNotValid" class="q-ml-md text-negative"
          >Ci sono dei campi obbligatori non validi</label
        >
        <label v-if="tipoSoggNotValid" class="q-ml-md text-negative"
          >Il campo <strong>tipo soggetto</strong> non ̬ valido</label
        >
    ...
    <ges-strutture-res-select-subject
      ref="selectSubject"
      @selectedSubject="onSubjectSelected"
    />
  </q-page>
</template>
...
import { isBeforeDate, notTooOldDate } from "../services/utils";
import { APP_CODE, GEST_EROGATORI } from "../services/business_logic";
...
export default {
  name: "PageNuovoOspitePersonale",
  components: {
    GesStruttureResSelectSubject,
    /* GesStruttureSegnalaSISPNuovoSoggetto, */ GesStrutturePageTitle
  },
  data() {
    return {
  ...
  computed: {
    ...
    comuneObbligatorio() {
      return !this.isAsrDomicileVisible;
    },
    ...
    subjectTypeOptions() {
      const result = this.subjectTypeList;
      sortBy(result, "descTipoSoggetto");

      result.push(NO_SUBJECT_TYPE);
  ...
  methods: {
    ...
    async save() {
      ...
      const isValid = await this.$refs.formSoggetto.validate();
          ...
          if (this.isSubjectOnSystem) {
            if (
              ...
              try {
                ...
                let message = "Non ̬ stato possibile aggiornare il soggetto";
  ...
  async created() {
    ...
    try {
      ...
      subjectTypeList.forEach(subjectType => {
        this.subjectTypeList.push(subjectType);
        if (
          subjectType.idTipoSoggetto === 3 ||
          subjectType.idTipoSoggetto === 4 ||
          subjectType.idTipoSoggetto === 20
        ) {
          this.subjectTypeListFilter.push(subjectType);
        }
      });
    } catch (err) {
