...
<template>
  <q-page padding>
    <ges-strutture-page-title ref="title" @back="onBack">Nuovo ospite o personale {{ nomeStruttura
    ...
    <q-form v-if="isSearchedOnAura && !isSearchingOnAura" ref="formSoggetto" greedy @submit="noop">
      <q-card class="q-mt-md">
        ...
        <label v-if="tipoSoggNotValid" class="q-ml-md text-negative">Il campo <strong>tipo soggetto</strong> non ̬
          valido</label>
        <q-toggle v-model="segnalaSISP" label="Segnala al sisp" />
        <ges-strutture-segnala-s-i-s-p-nuovo-soggetto ref="sisp" v-if="segnalaSISP">
        </ges-strutture-segnala-s-i-s-p-nuovo-soggetto>
        <!-- AZIONI -->
        <!-- ------ -->
    ...
    <template v-if="isOspite">
      ...
      <movimenti-dialog v-model="isOspite" :subject="subject" :idStrutturaRes="selectedStructure.idStruttura"
        @newAssociation="onNewAssociation" :title="'Movimento ingresso in struttura'"
        :tipologia="'NEW_ASSOCIATION_STRUCTURE'" :newMovimento="newMovimento">
      </movimenti-dialog>
    </template>
  </q-page>
</template>
...
import { APP_CODE, GEST_ST_RES } from "../services/business_logic";
